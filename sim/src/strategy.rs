...
pub fn move_to_most_efficient_ignore_high(r: &mut RobotInner, s: &mut SimState) -> Option<Box<dyn Action>> {
    //println!("Robot position, goal position and held item: {}, {:?}, {:?}",r.get_pos(), r.get_goal_pos(), r.get_item());
    if let None = r.get_item() {
        if s.can_give_cone(r) {
            //println!("Created an action to pick up a cone");
        ...
        if can_place {
            ...
            let j = s.most_efficient_junction_ignoring(&r, |j| *j.get_level() != Level::High);
            if !(j.is_none() || *j.as_ref().unwrap().get_level() == Level::Low) {
                println!("Found junction: {:?}", j.as_ref().unwrap().get_level());
                panic!();
            }
            if let Some(j) = j {
                r.set_goal_pos(*j.get_pos());
...
pub fn move_to_high(r: &mut RobotInner, s: &mut SimState) -> Option<Box<dyn Action>> {
    //println!("Robot position, goal position and held item: {}, {:?}, {:?}",r.get_pos(), r.get_goal_pos(), r.get_item());
    if let None = r.get_item() {
        if s.can_give_cone(r) {
            //println!("Created an action to pick up a cone");
        ...
        if can_place {
            ...
            let j = s.closest_junction_ignoring(&r, |junc| *junc.get_level() == Level::High);
            if let Some(j) = j {
                r.set_goal_pos(*j.get_pos());
                r.set_goal_angle(r.get_pos().angle_to(*j.get_pos()));
            }
